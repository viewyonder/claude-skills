...
`rm` on a symlink deletes the link, not the target — the source files in this repo are untouched, so you can re-install later by re-running the `ln -s` commands.
...
Neither `handoff` nor `pickup` should appear. If you see them with a strikethrough or "broken link" indicator, the symlink is still there pointing at a missing target — re-run the `rm` commands above.

Optionally, also clear the pointer file and any handoffs written to the default location:

```bash
rm -rf ~/.claude/handoffs
```

Skip this if you might re-install later and want `/pickup` to still find your most recent handoff. Handoffs written to `/tmp` (the default) will get cleaned up by the OS regardless.

Restart any running Claude Code sessions for the slash-command autocomplete to drop the entries.
